...
- **智能问答** — RAG + LLM，自动查询改写 + 多路检索
- **纯聊天** — 无工具无知识库的闲聊模式
- **知识库检索** — 直接搜索知识库
- **自定义模式** — 创建专属客服场景（自动生成 Prompt + 问候语）
- **流式输出** — SSE 流式 + Markdown 渲染，支持中途停止
- **历史对话** — 侧栏加载/切换历史会话，按模式分类
...
- 📊 知识库结构树（分类 → 文件 → Chunk）
- 📤 上传/删除/改分类/编辑 Chunk
- 📝 在线编辑 Prompt 模板 + 问候语/标题/副标题/输入提示
- ➕ 新建自定义客服模式（LLM 自动生成 Prompt 和文本）
- 💬 三种模式问候语可在线编辑（存数据库）

### 用户系统

- 对话持久化（按会话存储，支持按模式筛选）
- 用户画像（LLM 结构化提取 + 关键词规则兜底）
- 反馈收集（点赞/踩 + 统计）
